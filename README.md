...
テキストファイルまたは標準入力から1行ずつ読み込み、各パターンとのマッチングを行う。  
マッチングが成功した場合は、アクションが実行される。
...
```
         ...
         11 } | awk '
         12
         13   NR == 1 { # 現在読み込んでいる行が1行目の場合
         14     print "1行目を読み込みました..."
         15     print "1行目の内容「" $0 "」"
         16   }
         17
         18   $3 == "4" { # 3フィールド目が「4」の場合
         19     print "3フィールド目が「" $3 "」"
         20     print "内容「" $0 "」"
         21   }
         22 '
